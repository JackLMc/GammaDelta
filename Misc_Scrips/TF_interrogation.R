...
TF <- read.csv("./Data/TFCheckpoint.csv")
...
write.csv("./Output/TF/VD1_DGE_TF.csv", x = VD1b, row.names = F)
...
write.csv("./Output/TF/CD8_DGE_TF.csv", x = CD8b, row.names = F)
...
write.csv("./Output/TF/VD2_CD27LO.csv", x = tf_VD2_CD27LO)
write.csv("./Output/TF/VD2_CD27HI.csv", x = tf_VD2_CD27HI)
...
pdf(file = "./Figures/Transcription_Factor/VD1_TF_heatmap.pdf", height = 8, width = 8)
