# Single cell analysis of gamma-delta cells #
...
#### It's also based on the initial exploratory analyes done elsewhere.
...
#### We've lost 1030 rows because they're strange entries which aren't really genes.
...
#### Seurat performs normalisation on the data that you pass it.
#### This is more intended for counts and I don't want to use it for the TPM values.
#### So I'm going to set do.logNormalize to FALSE and then pass in log-transformed TPMs
#### This no longer works, transform and don't transform later. 
tpms <- log(tpms + 1)
...
mito.genes <- grep(pattern = "^MT-", x = rownames(x = gd.data@data), value = T)
length(mito.genes)
...
table(gd.data@meta.data$percent.mito < 0.05)
gd.data <- FilterCells(object = gd.data,
...
#### So, using the `RegressOut` function to do this. 
#### `RegressOut` Function is deprecated. Use scale data
